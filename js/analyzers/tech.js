const TechAnalyzer = {
  ...
  analyze(doc, html) {
    const detected = [...new Set(this.sigs.filter(s=>s.p.test(html)).map(s=>s.name))];
    const gen = doc.querySelector('meta[name="generator"]')?.getAttribute('content')||'';
    if (gen) detected.push(gen);
    const scripts = [...doc.querySelectorAll('script[src]')];
    return { detected, scriptsCount: scripts.length, scriptSrcs: scripts.map(s=>s.getAttribute('src')).filter(Boolean).slice(0,12) };
  }
};
